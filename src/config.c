...
Boolean load_config(char* path)
{
	...
	while ((str = fgets(line, 128, fp)) != NULL) {
		...
		} else if (strcasecmp(key, "LNAKAKEY")==0) {
			kc = keyname_to_code(value);
			if (kc != 0) {
				printf("LNAKAKEY: %s\n", value);
				set_left_nakakey(kc);
			}
		} else if (strcasecmp(key, "RNAKAKEY")==0) {
			kc = keyname_to_code(value);
			if (kc != 0) {
				printf("RNAKAKEY: %s\n", value);
				set_right_nakakey(kc);
			}
		} else if (strcasecmp(key, "LKOKEY")==0) {
			kc = keyname_to_code(value);
			if (kc != 0) {
				printf("LKOKEY: %s\n", value);
				set_left_kokey(kc);
			}
		} else if (strcasecmp(key, "RKOKEY")==0) {
			kc = keyname_to_code(value);
			if (kc != 0) {
				printf("RKOKEY: %s\n", value);
				set_right_kokey(kc);
			}
		} else if (strcasecmp(key, "OYAKEY")==0) {
			kc = keyname_to_code(value);
			if (kc != 0) {
				printf("OYAKEY: %s\n", value);
				set_oyakey(kc);
...
Boolean save_config(char *path)
{
	...
	fprintf(fp, "\n");
	fprintf(fp, "# シフトキー　\n");
	fprintf(fp, "LNAKAKEY=D\n");
	fprintf(fp, "RNAKAKEY=K\n");
	fprintf(fp, "LKOKEY=A\n");
	fprintf(fp, "RKOKEY=SEMICOLON\n");
	fprintf(fp, "OYAKEY=SPACE\n");
	fprintf(fp, "\n");
